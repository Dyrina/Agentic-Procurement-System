"""
api/sse.py — Per-session asyncio.Queue management and SSE event helpers.

Provides a lightweight layer for streaming Server-Sent Events back to a
connected client.  Each chat session gets its own ``asyncio.Queue``, and
helper functions make it easy to emit typed events from any agent node.
"""
